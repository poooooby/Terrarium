...
-- ------- lateral safety margin for floating world-hung UI
--
-- letterboxFov above keeps the VERTICAL framing pinned to the GB
-- reference no matter the window's shape. The horizontal axis has no
-- matching pin: BattleCapsule, BattlePanelsXY and BattleFanXY all hang
-- their pieces at fixed WORLD-space offsets along the camera's own right
-- axis, tuned by eye against a comfortably wide window. Because the
-- projection's own aspect is the window's raw pw/ph (see viewProjection's
-- caller), a narrow window gives those pieces a narrower slice of screen
-- to land in, and a fixed offset that read as "near the edge, but on" for
-- a wide window can walk clean past a narrow one's edge.
--
-- pw / (GB_W * s) counts how many whole GB-reference widths (see
-- fitScale) the real window covers -- 1.0 at the tightest a fit ever
-- gets, more on any window with bonus width beyond an exact multiple of
-- one. The window this was tuned against clears about 1.5 of those
-- comfortably; short of that, callers scale their own lateral offsets
-- down toward the camera's own centreline so the same pieces settle
-- further from the edge the narrower the window gets. A window already
-- at or past the reference is untouched.
BattleScene.LATERAL_REF = 1.5
BattleScene.LATERAL_FLOOR = 0.3

-- Cubed rather than linear: a panel overhangs the anchor by its own half
-- width on top of the offset, so recentring the anchor by the same
-- fraction the slack fell short by isn't enough to pull a wide panel's
-- own far edge back in -- pushing the curve down harder below the
-- reference clears that overhang without touching anything at or above
-- it (still exactly 1 there).
function BattleScene.lateralScale()
  local Renderer = require("src.render.Renderer")
  local pw = BattleScene.pixelSize()
  local s = Renderer:fitScale()
  if s <= 0 then return 1 end
  local ratio = (pw / (BattleScene.GB_W * s)) / BattleScene.LATERAL_REF
  if ratio > 1 then return 1 end
  local scale = ratio * ratio * ratio
  if scale < BattleScene.LATERAL_FLOOR then return BattleScene.LATERAL_FLOOR end
  return scale
end
